...
import { habits, entries, habitTags, FREQUENCY_VALUES } from '../db/schema.ts';
...
export const createHabitSchema = z.object({
  ...
  frequency: z.enum(FREQUENCY_VALUES).optional().default('daily'),
...
export type HabitIdParams = z.infer<typeof habitIdSchema>;
...
export const createHabit = async (
  req: AuthenticatedRequest<{}, {}, CreateHabitRequest>,
  res: Response,
) => {
  try {
...
export const getHabitById = async (
  req: AuthenticatedRequest<HabitIdParams>,
  res: Response,
) => {
  try {
    const { id } = req.params;
...
export const getHabitsByTag = async (
  ...
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch habits by tag' });
  }
};
...
export const addTagsToHabit = async (
  ...
  try {
    ...
    return res.status(500).json({ error: 'Failed to add tags to habit' });
  }
};
...
export const removeTagFromHabit = async (
  ...
  try {
    ...
    return res.status(500).json({ error: 'Failed to remove tag from habit' });
  }
};
