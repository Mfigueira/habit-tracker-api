...
async function seed() {
  ...
  try {
    ...
    const [healthTag] = await db
      .insert(tags)
      .values({ name: 'Health', hexColor: '#10B981' })
      .returning();

    const [_productivityTag] = await db
      .insert(tags)
      .values({ name: 'Productivity', hexColor: '#3B82F6' })
      .returning();
    ...
    console.log('\n📊 Seed Summary:');
    console.log(`- Demo user has ${userWithHabits?.habits.length || 0} habit`);
    console.log('\n🔑 Login Credentials:');
